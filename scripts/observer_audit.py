...
import numpy as np
ROOT=Path(__file__).resolve().parents[1];DATA=ROOT/'derived/world_vectors_v0.2.npz';OUT=ROOT/'reports/checkpoint_observer_audit_v0.2.json';MD=ROOT/'reports/checkpoint_observer_audit_v0.2.md';SEED=420042
...
def main():
 ...
 lines=['# Checkpoint — Observer Audit','',f'- Blocks: **{len(ub)}**',f'- Block pairs: **{len(pairs)}**',f'- Observable pairs with |r| ≥ 0.90 shown: **{len(tops)}**','','## Block dependence','','| Block A | Block B | RV | Null q95 | Above null? |','|---|---|---:|---:|---|']
 ...
 lines += ['','## Strong observable-level relations','','| Observable A | Observable B | r |','|---|---|---:|']
