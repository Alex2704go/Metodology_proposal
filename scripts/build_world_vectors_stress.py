...
DATA=ROOT/'derived/objects_blind_stress_4004.jsonl.gz'
OUT=ROOT/'derived/world_vectors_stress_4004.npz'; MAN=ROOT/'derived/world_vectors_stress_4004_manifest.json'; REPORT=ROOT/'reports/checkpoint_vector_qc_stress_4004.md'
SEED=420042
...
def main():
 ...
 lines=['# Checkpoint — WORLD Vector QC','',f'- Objects: **{n}**',f'- Candidate organizational observables: **{raw_count}**',f'- Retained organizational observables: **{len(names)}**',f"- Non-finite values after processing: **{manifest['nonfinite_after_processing']}**",f"- Duplicate vector rows beyond first: **{manifest['duplicate_vector_rows_beyond_first']}**",'', '## Blocks','']+[f'- `{k}`: {v}' for k,v in sorted(manifest['block_counts'].items())]+['','## Leakage checks','']+[f"- `{k}`: **{v}**" for k,v in manifest['forbidden_feature_checks'].items()]+['','## Removed exact duplicates','']+[f"- `{x['removed']}` = `{x['kept']}`" for x in dups]+['','## Removed within-block |r| >= 0.995','']+[f"- `{x['removed']}` ≈ `{x['kept']}`" for x in corr_removed]+['','## Removed constants','']+[f'- `{x}`' for x in removed_constants]
 REPORT.write_text('\n'.join(lines)+'\n',encoding='utf-8')
