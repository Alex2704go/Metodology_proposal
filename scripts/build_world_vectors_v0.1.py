...
OUT=ROOT/'derived/world_vectors_v0.1.npz'; MAN=ROOT/'derived/world_vectors_v0.1_manifest.json'; REPORT=ROOT/'reports/checkpoint_vector_qc.md'
SEED=420042
...
def add_numeric(cols,names,blocks,name,block,vals):
 a=np.asarray(vals,dtype=float);bad=~np.isfinite(a)
 if bad.any():
  good=a[~bad];fill=float(np.median(good)) if len(good) else 0.;a[bad]=fill
  cols.append(bad.astype(float));names.append(name+'__MISSING');blocks.append(block)
...
def main():
 ...
 add_numeric(cols,names,blocks,'composition.total','composition_shape',[sum(x.values()) for x in comps])
 for stat in ['entropy','max_share','min_share','l2']:
  vals=[]
  for x in comps:
   a=np.asarray(list(x.values()),float);p=a/a.sum() if len(a) and a.sum()!=0 else np.zeros(len(a))
 ...
 for j in range(maxk):
  vals=[]
  for x in comps:
   a=np.asarray(list(x.values()),float);p=sorted((a/a.sum()).tolist(),reverse=True) if len(a) and a.sum()!=0 else []
